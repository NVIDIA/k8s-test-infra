...
// It does NOT implement nvml.Interface - it delegates to a MockServer.
type Engine struct {
	server    *MockServer
	config    *Config
	handles   *HandleTable
...
func (e *Engine) Init() nvml.Return {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Failed to create mock server: %v\n", err)
...
func (e *Engine) applyConfig(server *dgxa100.Server) {
	...
	if e.config.NumDevices != 8 {
		...
		server.DeviceGetCountFunc = func() (int, nvml.Return) {
			// dgxa100 has 8 devices max, clamp to that
			if numDevices <= 8 {
