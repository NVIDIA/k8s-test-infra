...
func TestHandleTable_Lookup(t *testing.T) {
	...
	zero := ht.Lookup(0)
	if zero != nil {
		t.Error("Expected nil for zero handle")
	}
}
...
func TestHandleTable_Clear(t *testing.T) {
	...
	// Lookup should fail after clear
	if ht.Lookup(handle1) != nil {
		t.Error("Expected nil after clear")
	}
	if ht.Lookup(handle2) != nil {
		t.Error("Expected nil after clear")
	}
}
	...
	for i := 0; i < numGoroutines; i++ {
		go func(id int) {
			...
			for j := 0; j < devicesPerGoroutine; j++ {
				...
				handle := ht.Register(dev)
				if handle == 0 {
					t.Errorf("Got zero handle in goroutine %d", id)
				}
			}
		}(i)
	}
	...
	for i := 0; i < numGoroutines; i++ {
		...
		go func() {
			...
			for j := 0; j < 100; j++ {
				...
				if dev == nil {
					t.Error("Lookup returned nil for valid handle")
				}
			}
		}()
		...
		go func(id int) {
			...
			handle := ht.Register(dev)
			if handle == 0 {
				t.Error("Register returned zero handle")
			}
		}(i)
	}
	wg.Wait()
