...
// a100SupportedProfiles is the A100-SXM4-40GB profile table, read out of the
// profile the chart ships. A board is MIG-capable only if it declares a table,
// so every test device that partitions needs one, and the A100's is the
// reference: NVIDIA's published values for it are what the mock already
// reported before this data moved into YAML.
//
// It is read rather than restated because a row now declares its placements,
// its reported id and its whole compute-instance listing, and a second copy of
// that geometry would be a second thing to keep correct. Reading it also means
// every device these fixtures build partitions the way the shipped board does.
func a100SupportedProfiles() []MIGProfileSpec {
	return shippedProfileMIG("a100").SupportedProfiles
}
