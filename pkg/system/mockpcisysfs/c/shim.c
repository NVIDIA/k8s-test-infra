 ...
 * fake tree under $MOCK_PCI_ROOT.
 *
 * Real consumers (lspci, libpciaccess, k8s deviceattribute, ...) read from
 * /sys/bus/pci/... and /sys/devices/pciDDDD:BB/... We hook the libc syscall wrappers,
 * and for any path starting with one of those prefixes we splice
 * $MOCK_PCI_ROOT in front and forward to the next libc.
 *
 * Design notes:
 ...
 *   - When MOCK_PCI_ROOT is unset or empty the shim is a true no-op so the
 *     process sees the real host sysfs.
