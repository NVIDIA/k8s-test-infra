...
All profiles use the same custom Kind node image (`kind-node-nv:latest`) built from `deployments/kind-nvidia-cdi/Dockerfile`, which pre-installs the NVIDIA container runtime and enables CDI and NRI in containerd.
