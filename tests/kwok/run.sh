...
inventory_summary_ready() {
	...
	kctl get sgpuinventory mokka-kwok -o json | jq -e \
		...
		.status.capacity.nodes == $capacity and
		...
		([.status.conditions[] | select(
			(.type == "Accepted" or .type == "ResolvedRefs" or .type == "Programmed") and
			.status == "True"
		)] | length) == 3 and
		any(.status.conditions[]; .type == "Programmed" and .observedGeneration == $generation) and
...
capture_failure() {
	...
	kctl get sgpurackprofiles,sgpuinventories,sgpuracks,sgpuruntimepolicies,nodes,leases -A -o json >"${ARTIFACT_DIR}/failure.resources.json" 2>&1
...
(( NODE_COUNT >= 2 * NODES_PER_RACK )) || die "KWOK_NODE_COUNT must span at least two racks for shrink/grow scenarios"
...
go build -o "${CONTROLLER_BIN}" ./cmd/control-plane
...
kctl wait --for=condition=Established --timeout=2m crd/sgpurackprofiles.mokka.nvidia.com crd/sgpuinventories.mokka.nvidia.com crd/sgpuracks.mokka.nvidia.com crd/sgpuruntimepolicies.mokka.nvidia.com
...
readonly SHRUNK_RACKS="$((FULL_RACKS / 2))"
...
check_state inventory-shrink "${scenario_started_seconds}" "${SHRUNK_RACKS}" "${TOTAL_WITH_EXTRA}" "${TOTAL_WITH_EXTRA}" "$((SHRUNK_RACKS * NODES_PER_RACK))" false
scenario_started_seconds="$(date -u +%s)"
apply_inventory "${FULL_RACKS}"
