...
// Command kwok-assert checks the scale scenario's bounded controller contract.
package main
...
//nolint:cyclop // Each input file has an independent parse and type failure mode.
...
//nolint:cyclop // The command accumulates all independent scale-contract violations in one report.
func check(o options, inventory *mokkav1alpha1.SGPUInventory, racks *mokkav1alpha1.SGPURackList, nodes *corev1.NodeList) result {
	...
	capacity := int32(o.expectedRacks * o.nodesPerRack)
	pending := int32(o.expectedEligible - o.expectedAllocated)
	if inventory.Name != "mokka-kwok" || inventory.UID == "" {
		addError("unexpected inventory identity %q/%q", inventory.Name, inventory.UID)
	}
	if inventory.Status.Capacity.Racks != int32(o.expectedRacks) ||
		inventory.Status.Capacity.Nodes != capacity || inventory.Status.Capacity.GPUs != capacity {
		addError("inventory capacity is %+v, want racks=%d nodes=%d GPUs=%d",
			inventory.Status.Capacity, o.expectedRacks, capacity, capacity)
	}
	wantAvailable := capacity - int32(o.expectedAllocated)
	usage := inventory.Status.Usage
	if usage.RequestedNodes != int32(o.expectedEligible) ||
		usage.AllocatedNodes != int32(o.expectedAllocated) ||
		usage.AvailableNodes != wantAvailable || usage.PendingNodes != pending {
		addError("inventory usage is %+v, want requested=%d allocated=%d available=%d pending=%d",
			usage, o.expectedEligible, o.expectedAllocated, wantAvailable, pending)
	}
	for _, conditionType := range []string{"Accepted", "ResolvedRefs", "Programmed"} {
		checkCondition(addError, inventory.Status.Conditions, conditionType, metav1.ConditionTrue)
	}
	if programmed := findCondition(inventory.Status.Conditions, "Programmed"); programmed == nil ||
		programmed.ObservedGeneration != inventory.Generation {
		addError("inventory generation %d is not programmed", inventory.Generation)
	}
