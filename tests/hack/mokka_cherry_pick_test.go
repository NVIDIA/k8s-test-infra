...
func TestMokkaCherryPickDoesNotRestrictPullRequestCommitCount(t *testing.T) {
	_, script := mokkaPaths(t)
	for _, source := range []string{
		...
		t.Run(source, func(t *testing.T) {
			result := runMokkaDriver(t, script, validMokkaInputs(), map[string]string{"FAKE_SOURCE": source})
			require.NoError(t, result.err, result.output)
			require.True(t, mokkaSuccessMutationLogsAreExact(result.gitLog, result.ghCallLog), "valid squash merge did not use the reviewed mutation sequence:\ngit:\n%s\ngh calls:\n%s", result.gitLog, result.ghCallLog)
		})
	}
}
