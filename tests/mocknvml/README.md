...
- `nvmlDeviceGetCudaComputeCapability()` - Get compute capability (stub - returns `ERROR_NOT_SUPPORTED`)
...
- Some NVML functions return `ERROR_NOT_SUPPORTED` (381 out of 396 functions are stubs)
- Compute capability queries are not yet supported (returns `ERROR_NOT_SUPPORTED`)
