...
"Status" means exactly what CI does, not what the suite is capable of. Helm
charts installed directly by the suite are pinned: GPU Operator
(`local/gpu-operator/gpu_operator.tiltfile`), Node Feature Discovery
(`scenario_nfd_test.go:126`), and the DRA driver
(`local/dra/dra.tiltfile`). GPU Operator operand versions are inherited from
the pinned chart.
...
| NVIDIA Device Plugin (GPU Operator operand) | inherited from GPU Operator v26.7.0 | from the `gpu-operator` chart | Pinned through the chart; runs in CI |
...
| GPU Operator | v26.7.0 | `nvidia/gpu-operator` (Helm) | **Pinned** + runs in CI |
...
| DCGM Exporter (GPU Operator operand) | inherited from GPU Operator v26.7.0 | from the `gpu-operator` chart | Pinned through the chart; runs in CI |
...
- **GPU Operator** (pinned Helm chart + values overlay): its own device plugin, GFD, dcgm-exporter and validator operands (`e2e-gpu-operator`, 6 profiles) — see the overlay section below
