...
- **Managed driver** (baseline + `tests/e2e/go/assets/gpu-operator-driver-values.yaml`
  ...
  operator version whose contract is vendored under `contract/`. This is a
  lifecycle test, not driver functionality (DCGM/MIG/upgrades remain
  uncovered). DCGM exporter is disabled here for the same reason as the
  hostDriver lane.
