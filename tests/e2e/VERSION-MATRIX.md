...
Operator chart (`local/gpu-operator/gpu_operator.tiltfile`) and Node Feature
Discovery (`scenario_nfd_test.go`) are pinned. The DRA driver chart carries no
`--version`, so its version **floats**. The Operator's operands carry no pin of
their own; their versions are that chart's defaults.
...
This release of the mock does not simulate kernel modules, so no container sees
that line, and GFD, the device plugin and dcgm-exporter wait forever.
