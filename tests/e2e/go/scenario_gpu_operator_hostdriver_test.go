...
// hostResiduePaths are the node paths (globs) the host-driver masquerade
// installs. After `helm uninstall`, none may remain -- the manifest cleanup
// must remove exactly what setup wrote. Globs (not exact names) cover the
// version-suffixed library files (libnvidia-ml.so.<ver>, libcuda.so.<ver>) and
// the per-GPU device nodes without duplicating drl_lib_filenames here. Kept a
// var so the unit test can pin the covered set.
...
// stdout == clean uninstall). Missing files and unmatched globs go to stderr,
// which is discarded, so only survivors reach stdout. The trailing `true`
// pins the compound exit status to 0: otherwise the last `ls` of an absent
// path (the normal clean case) would exit non-zero and be misread as a
// command failure. Emptiness of stdout -- not the exit code -- is the signal.
func hostResidueCheckCmd() string {
	return "ls -d " + strings.Join(hostResiduePaths, " ") + " 2>/dev/null; true"
}
