...
const (
	...
	imexStagedConfig    = "/opt/nvml-mock/driver/etc/nvidia-imex/config.cfg"
)
...
func assertIMEXLifecycle(ctx SpecContext, h *harness.Harness, workers []cluster.Node) {
	...
	Eventually(func() (string, error) {
		...
		ShouldNot(Equal("UP"), "a domain with only one of its two configured peers must not report UP")
...
sed -e 's/^DAEMONIZE=1/DAEMONIZE=0/' \
    ...
    %s > %s`, ipA, ipB, imexLifecycleNodes,
		imexLifecycleNodes, imexStagedConfig, imexLifecycleConfig)
