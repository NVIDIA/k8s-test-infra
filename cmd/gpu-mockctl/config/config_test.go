...
func TestNewDefault(t *testing.T) {
	...
	if cfg.DriverRoot != "/var/lib/nvidia-mock/driver" {
		t.Errorf("Expected driver-root=/var/lib/nvidia-mock/driver, got %s", cfg.DriverRoot)
	}
	if cfg.CDIOutput != cdi.DefaultSpecPath {
		t.Errorf("Expected cdi-output=%s, got %s", cdi.DefaultSpecPath, cfg.CDIOutput)
	}
	if cfg.Machine != "dgxa100" {
		t.Errorf("Expected machine=dgxa100, got %s", cfg.Machine)
	}
	if cfg.ToolkitRoot != "/usr/local/nvidia-container-toolkit" {
		t.Errorf("Expected toolkit-root=/usr/local/nvidia-container-toolkit, got %s", cfg.ToolkitRoot)
	}
...
func TestValidateCDI(t *testing.T) {
	tests := []struct {
		name    string
		cfg     *Config
		wantErr bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := tt.cfg.ValidateCDI()
			if (err != nil) != tt.wantErr {
				t.Errorf("ValidateCDI() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
