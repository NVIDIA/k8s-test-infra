...
// ValidateCDI validates configuration for CDI mode
func (c *Config) ValidateCDI() error {
	if c.DriverRoot == "" {
		return fmt.Errorf("driver-root path cannot be empty")
	}
	if c.CDIOutput == "" {
		return fmt.Errorf("cdi-output path cannot be empty")
	}
	if c.ToolkitRoot == "" {
		return fmt.Errorf("toolkit-root path cannot be empty")
	}
	return c.Validate()
}
