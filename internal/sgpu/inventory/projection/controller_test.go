...
func TestProjectAppliesOnlyOwnedMetadataWithExactAssignment(t *testing.T) {
	...
	require.Equal(t, types.UID("node-uid"), decoded.NodeUID)
...
func TestProjectRejectsUnauthorizedReplacementAndClearsPriorConflict(t *testing.T) {
	...
	controller := NewController(cache, patcher)
	conflict := outcomeFor(rack, &rack.Spec.Nodes[0])
	conflict.State, conflict.Reason = StateConflict, ReasonNodeMetadataConflict
...
func TestProjectSkipsApplyForExactOwnedProjection(t *testing.T) {
	...
	require.Empty(t, patcher.calls)
	require.Empty(t, controller.Outcomes(), "the fast path must not retain successful outcomes")
}
...
func TestProjectRejectsDuplicateBindingsAndExactUIDReplacement(t *testing.T) {
	...
	require.Empty(t, patcher.calls)
	require.Empty(t, controller.Outcomes(), "duplicate binding outcomes are not used by projection status")
...
func TestCleanupRetryClearsMetadataConflictOnGenericError(t *testing.T) {
	rack := testRack()
	node := testNode("node", "node-uid")
	setExactProjection(t, node, rack)
	setManagedFields(node, FieldManager, []string{inventorymetadata.AssignedLabel, inventorymetadata.CliqueLabel}, []string{inventorymetadata.AssignmentAnnotation})
	...
	patcher := &recordingPatcher{
		err: apierrors.NewConflict(schema.GroupResource{Resource: "nodes"}, node.Name, errors.New("owned elsewhere")),
	}
	controller := NewController(cache, patcher)
	cleanup := cleanupFor(rack)
	...
	require.Empty(t, controller.Outcomes(), "a conclusive non-conflict cleanup retry clears the prior marker")
}
...
func TestProjectionStateDoesNotRetainSuccessfulBindings(t *testing.T) {
	const churn = 4_000
	...
	require.Zero(t, outcomes, "successful projections must not consume retained status state")
	require.Zero(t, cleanups)
	require.Empty(t, controller.Outcomes())
}
		...
		for rackIndex := range racksPerInventory {
			...
			for nodeIndex := range nodesPerRack {
				controller.record(Outcome{
					...
					State:         StateConflict,
					Reason:        ReasonNodeMetadataConflict,
				})
			}
		}
	}
...
func TestRevokeCleanupRemovesOnlyExactAcknowledgement(t *testing.T) {
	rack := testRack()
	node := testNode("node", "node-uid")
	cache := &fakeCache{
		nodes: map[string]*corev1.Node{node.Name: node},
	...
	require.Equal(t, cleanup, acknowledged)

	other := cleanup
	other.Binding.Node.UID = "other-node-uid"
	_, ready = controller.AcknowledgedCleanup(rack.Name, other.Binding)
	require.False(t, ready)
	controller.RevokeCleanup(other)
	require.True(t, controller.Ready(cleanup), "a different exact binding must not consume the acknowledgement")

	controller.RevokeCleanup(cleanup)
	require.False(t, controller.Ready(cleanup))
}
