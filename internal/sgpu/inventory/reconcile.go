...
// Package inventory reconciles inventories into durable rack resources using
...
func (r *Reconciler) reconcile(ctx context.Context, key string, requestedGroup *allocate.RackGroupKey) (Result, error) {
	...
	for _, group := range workGroups {
		...
		reconcileRack := func(rackIndex int32) error {
			...
			if err := rackrender.ValidateRackSpec(targetSpec); err != nil {
...
func (r *Reconciler) resolveGroups(inventory *mokkav1alpha1.SGPUInventory) ([]resolvedGroup, []ProfileIssue, error) {
	...
	for _, group := range inventory.Spec.RackGroups {
		...
		if err := rackrender.ValidateProfile(profile.Spec); err != nil {
...
func validateGroupMaterialization(
	...
	issues := make([]ProfileIssue, 0)
	for _, group := range groups {
		_, err := rackrender.RenderRackWithRevision(rackrender.RackInput{
			InventoryName: inventory.Name,
			InventoryUID:  inventory.UID,
			Group:         group.group,
			RackIndex:     0,
