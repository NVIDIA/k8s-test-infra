...
const (
	...
	ReasonCapacityExceeded      = sgpuinventory.ReasonCapacityExceeded
...
type groupAggregate struct {
	...
	capacity  sgpuinventory.DeclaredCapacity
...
func ComputeInventory(input InventoryInput, now metav1.Time) mokkav1alpha1.SGPUInventoryStatus {
	...
	issues := make(map[string]sgpuinventory.ProfileIssue, len(input.RackResult.ProfileIssues))
	...
	totalCapacity := sgpuinventory.DeclaredCapacity{}
	capacityValid := true
	for _, declaration := range inventory.Spec.RackGroups {
		...
		if profile != nil && !hasIssue {
			...
			capacity, err := sgpuinventory.CapacityForGroup(declaration, profile)
			if err != nil {
				capacityValid = false
			} else {
				aggregate.capacity = capacity
				totalCapacity, err = sgpuinventory.AddCapacity(totalCapacity, capacity)
				if err != nil {
					capacityValid = false
				}
			}
	...
	slices.Sort(ordered)
	if capacityValid {
		if err := sgpuinventory.ValidateSupportedCapacity(totalCapacity); err != nil {
			capacityValid = false
		}
	}
	if capacityValid {
		for _, aggregate := range groups {
			...
			aggregate.status.Capacity, _ = sgpuinventory.StatusCapacity(aggregate.capacity)
		}
	}
	...
	if capacityValid {
		result.Capacity, _ = sgpuinventory.StatusCapacity(totalCapacity)
	}
