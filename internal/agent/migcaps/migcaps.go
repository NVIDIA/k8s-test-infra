...
// Stage materializes the MIG capability surface under the host root. It is a
// no-op (but marks ready) when no GPU boots partitioned, because a driver on
// which MIG was never enabled publishes none of these files, and their absence
// is how a consumer detects that.
func (s *Simulator) Stage(_ context.Context, state *agent.State) error {
	...
	if !state.MIG.Partitioned() {
		...
		if err := removeSurface(s.host); err != nil {
			return err
		}
		s.ready.Store(true)
		return nil
	...
	if err := stageCapDevs(s.host, state.MIG.CapsMajor, caps); err != nil {
		return fmt.Errorf("migcaps cap devices: %w", err)
	}
	if err := stageMinors(s.host, caps); err != nil {
		return fmt.Errorf("migcaps mig-minors: %w", err)
	}
	if err := stageCapabilityTree(s.host, caps); err != nil {
