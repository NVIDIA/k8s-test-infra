...
func TestComputeInventoryStatusExactAggregateMathAndConditions(t *testing.T) {
	...
	require.Equal(t, input.Inventory.Generation, got.ObservedGeneration)
	require.Equal(t, mokkav1alpha1.SGPUCapacity{Racks: 3, NodeSlots: 7, GPUs: 22}, got.Capacity)
	require.Equal(t, mokkav1alpha1.SGPUUsage{
		RequestedNodes: 5, AllocatedNodes: 5, AvailableNodes: 2,
		PendingNodes: 1, ConflictingNodes: 2, ProjectedNodes: 2,
	}, got.Usage)
	require.Equal(t, []mokkav1alpha1.SGPURackGroupStatus{
		{ID: "a", Capacity: mokkav1alpha1.SGPUCapacity{Racks: 2, NodeSlots: 4, GPUs: 16}, Usage: mokkav1alpha1.SGPUUsage{
			RequestedNodes: 4, AllocatedNodes: 3, AvailableNodes: 1, PendingNodes: 1, ConflictingNodes: 2, ProjectedNodes: 1,
		}},
		{ID: "b", Capacity: mokkav1alpha1.SGPUCapacity{Racks: 1, NodeSlots: 3, GPUs: 6}, Usage: mokkav1alpha1.SGPUUsage{
			RequestedNodes: 2, AllocatedNodes: 2, AvailableNodes: 1, ConflictingNodes: 2, ProjectedNodes: 1,
		}},
		{ID: "missing"},
	}, got.RackGroups)
	...
	require.Equal(t, metav1.ConditionFalse, condition(got.Conditions, mokkav1alpha1.SGPUInventoryConditionResolvedRefs).Status)
	require.Equal(t, ReasonProfileNotFound, condition(got.Conditions, mokkav1alpha1.SGPUInventoryConditionResolvedRefs).Reason)
	require.Equal(t, metav1.ConditionFalse, condition(got.Conditions, mokkav1alpha1.SGPUInventoryConditionMaterialized).Status)
	require.Equal(t, metav1.ConditionFalse, condition(got.Conditions, mokkav1alpha1.SGPUInventoryConditionRequestsSatisfied).Status)
	require.Equal(t, ReasonPlacementConflicts, condition(got.Conditions, mokkav1alpha1.SGPUInventoryConditionRequestsSatisfied).Reason)
	require.Equal(t, metav1.ConditionFalse, condition(got.Conditions, mokkav1alpha1.SGPUInventoryConditionNodesProjected).Status)
	require.Equal(t, ReasonProjectionIncomplete, condition(got.Conditions, mokkav1alpha1.SGPUInventoryConditionNodesProjected).Reason)
}
...
func TestComputeInventoryRequestedNodesAreDistinctWhileGroupsOverlap(t *testing.T) {
	...
	require.Equal(t, int64(1), got.Usage.RequestedNodes)
	require.Equal(t, int64(1), got.RackGroups[0].Usage.RequestedNodes)
	require.Equal(t, int64(1), got.RackGroups[1].Usage.RequestedNodes)
	require.Equal(t, int64(1), got.Usage.ConflictingNodes)
}
...
func TestComputeRackStatusCountsExactProjectionAndDuplicateBindings(t *testing.T) {
	...
	rack.Status.Conditions = []metav1.Condition{{
		Type: mokkav1alpha1.SGPURackConditionReady, Status: metav1.ConditionFalse,
	...
	require.Equal(t, int32(1), got.ProjectedSlots)
	require.Equal(t, metav1.ConditionFalse, condition(got.Conditions, mokkav1alpha1.SGPURackConditionReady).Status)
	require.Equal(t, ReasonDuplicateBindings, condition(got.Conditions, mokkav1alpha1.SGPURackConditionReady).Reason)
	require.Equal(t, oldTime, condition(got.Conditions, mokkav1alpha1.SGPURackConditionReady).LastTransitionTime)
	require.Equal(t, metav1.ConditionFalse, condition(got.Conditions, mokkav1alpha1.SGPURackConditionNodesProjected).Status)
	...
	require.Equal(t, metav1.ConditionTrue, condition(got.Conditions, mokkav1alpha1.SGPURackConditionReady).Status)
	require.Equal(t, metav1.ConditionTrue, condition(got.Conditions, mokkav1alpha1.SGPURackConditionNodesProjected).Status)
}
...
func TestProjectionErrorsRemainRetryableStatusInputs(t *testing.T) {
	...
	got := ComputeInventory(input, metav1.Now())
	projected := condition(got.Conditions, mokkav1alpha1.SGPUInventoryConditionNodesProjected)
	require.Equal(t, metav1.ConditionFalse, projected.Status)
...
func aggregateInput() InventoryInput {
	inventory := &mokkav1alpha1.SGPUInventory{
		...
		Spec: mokkav1alpha1.SGPUInventorySpec{RackGroups: []mokkav1alpha1.SGPURackGroup{
			{ID: "a", Count: 2, ProfileRef: corev1.LocalObjectReference{Name: "pa"}, Placement: placement("a", "both")},
			{ID: "b", Count: 1, ProfileRef: corev1.LocalObjectReference{Name: "pb"}, Placement: placement("b", "both")},
			{ID: "missing", Count: 4, ProfileRef: corev1.LocalObjectReference{Name: "missing"}, Placement: placement("missing")},
		}},
	}
...
func placement(values ...string) *mokkav1alpha1.SGPUPlacement {
	return &mokkav1alpha1.SGPUPlacement{NodeSelector: &metav1.LabelSelector{MatchExpressions: []metav1.LabelSelectorRequirement{{
...
func profile(name string, nodesPerRack, gpus int32) *mokkav1alpha1.SGPUProfile {
	return &mokkav1alpha1.SGPUProfile{
		...
		Spec: mokkav1alpha1.SGPUProfileSpec{
			...
			Node: mokkav1alpha1.SGPUProfileNode{GPUs: mokkav1alpha1.SGPUHardware{Count: gpus}},
		},
	}
}
