...
type Controller struct {
	...
	cleaned             map[bindingKey]controllerack.CleanupReason
...
func NewController(cache Cache, patcher NodePatcher) *Controller {
	return &Controller{
		...
		cleaned:             make(map[bindingKey]controllerack.CleanupReason),
...
// allocation decision.
func (c *Controller) RevokeCleanup(needed controllerack.CleanupNeeded) {
	operation := c.operationLock(needed.RackName, needed.Binding.Coordinate.NodeIndex)
	operation.Lock()
	defer operation.Unlock()
...
func (c *Controller) AcknowledgedCleanup(
...
) (controllerack.CleanupNeeded, bool) {
	c.mu.RLock()
	defer c.mu.RUnlock()
	...
	if !exists || key.inventoryName != binding.Coordinate.Group.InventoryName ||
		...
		return controllerack.CleanupNeeded{}, false
	}
