...
In scope of [MEP-0001 (Mokka Control Plane)](../0001-mokka-control-plane/README.md),
...
|-------------------------|-----------------------------------------------------------------|--------------------------------------------------------------------------------------------|----------------------------------------------------------------------|
| `driverfootprint`       | Component 1 — GPU driver footprint                              | chardevs, NVML shim, CUDA shim, nvidia-smi, procfs version+params, mock-NVML engine config | —                                                                    |
...
| `nodeidentity`          | Component 8 — K8s node identity                                 | node label + NFD feature file + `/run/nvidia/driver` symlink                               | —                                                                    |
...
```
                     ┌── driverfootprint (chardevs + libs + smi + procfs + engine config, all parallel)
                     ├── pcibus         (sysfs tree)
...
|---------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `/dev/nvidia0..N`, `/dev/nvidiactl`, `/dev/nvidia-uvm`, `/dev/nvidia-uvm-tools` char devices (majors 195/510) | K8s device-plugin NVML discovery, NRI plugin device injection, workload libs | ✓ `driverfootprint` — `mknod` with correct major/minor                                                                                                               |
| `libnvidia-ml.so.<version>` C ABI (~400 symbols)                                                              | `nvidia-smi`, DCGM, go-nvml consumers, K8s device-plugin                     | ✓ `driverfootprint` stages the shim; 89 real implementations in `pkg/gpu/mocknvml/engine` + auto-generated stubs. Coverage measurable via `generate-bridge --stats`. |
| `libcuda.so.<version>` C ABI (driver API)                                                                     | CUDA driver-API-only apps                                                    | ~ `driverfootprint` stages the shim; only 15 functions in `pkg/gpu/mockcuda`. Full CUDA runtime not covered.                                                         |
| `/proc/driver/nvidia/version`                                                                                 | `nvidia-smi` startup banner, GPU Operator validator                          | ✓ `driverfootprint`                                                                                                                                                  |
| `/proc/driver/nvidia/params`                                                                                  | GPU Operator driver-status probes                                            | ✓ `driverfootprint`                                                                                                                                                  |
...
| `nvidia-smi` ELF binary + shell fallback                                                                      | shell scripts, operator tooling that invokes `nvidia-smi`                    | ✓ `driverfootprint` stages real RPATH-patched ELF; shell fallback covers minimal cases                                                                               |
| Mock-NVML engine on-disk config (compiled `state` → engine config file the shim reads at dlopen)              | mock-NVML shim itself at container-run time                                  | ✓ `driverfootprint` — atomic write with `unix.Flock`; co-writer `nvml-mock-ctl` shares the lock                                                                      |
...
| Surface                                                                                                       | Consumers                                                                  | Coverage                                                                                                                                           |
|---------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------|
| `/sys/class/infiniband/mlx5_<N>/{fw_ver,board_id,hca_type,ports/<P>/{state,rate,gid_tbl_len,phys_state,lid}}` | Network Operator, `ibstat`, `ibv_devinfo`, DCGM fabric metrics, Topograph  | ✓ `ibhca` via `pkg/network/mockib/sysfs`                                                                                                           |
...
This logic is either not supposed to be a part of the Mokka Node Agent (Mokka Control Plane is a better place for node labeling) or it's supposed to be done via GFD.
