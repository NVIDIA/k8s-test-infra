...
| Kernel module presence: `/proc/modules`, `/sys/module/nvidia/{refcnt,coresize,initstate,version,holders/}`, `/sys/module/nvidia_uvm/` | `lsmod`, GPU Operator validator's `isNvidiaModuleLoaded()`, `modprobe` | ✓ `gpudriver` via `internal/kmod`. The agent mirrors the node's own `/sys/module` and adds the NVIDIA entries to the copy. The mirror includes built-in modules and their `parameters/`, because the mount replaces that tree for every reader. A host module therefore keeps its `lsmod` columns. Two channels deliver the surface. `libmockfs.so` redirects both paths for libc callers. Both CDI specs bind-mount `sys/module` at its kernel path for Go callers, which no `LD_PRELOAD` can reach. runc allows only an allowlist inside `/proc`, so the agent cannot bind-mount `/proc/modules`. CDI-served containers therefore get a generated `lsmod` that reads the mounted tree. A `State` reconcile refreshes the mirror. A module load or unload does not, so the mirror can lag the node |
