...
| `/proc/driver/nvidia/params`                                                                                  | `nvidia-modprobe` device-node ownership/permissions, GPU Operator driver-status probes | ✓ `gpudriver` — real unprefixed key names in the driver's own order, since `nvidia-modprobe`'s scan stops at the first line it cannot consume. Device-file UID/GID/mode come from the profile's `driver:` block |
| `/proc/driver/nvidia/gpus/<domain:bus:dev.fn>/information`                                                    | PCI-address-to-device-minor resolution (documented alternative to `nvmlDeviceGetMinorNumber`), container-visible GPU enumeration | ✓ `gpudriver` — one directory per device; `Device Minor` is cross-checked against the `mknod` minor and the engine's `nvmlDeviceGetMinorNumber` for every shipped profile |
| `/proc/driver/nvidia/registry`, `/proc/driver/nvidia/warnings`                                                | driver diagnostics                                                           | ✗ gap — diagnostics only; no known consumer gates on them                                                                                                            |
| `/proc/driver/nvidia/capabilities/mig/{config,monitor}`                                                       | MIG capability device-node discovery                                         | ✗ gap — blocked on MIG generally; the `fabric-imex-mgmt` writer already has the right `DeviceFileMinor/Mode/Modify` shape to reuse                                    |
...
**Restage trigger**: `state.Software`, `state.DriverParams` or `state.Devices` fields change.
