...
| `infiniband`                 | Component 5 — InfiniBand HCA                                    | IB sysfs tree + libibmock*.so staging + IB CLI tool staging                                | —                                       | `pkg/network/mockib/daemon.Server`; optional fabric relay            |
...
```
   ...
   State snapshot ───┼── imex           (chardevs + /proc/devices overlay) ──┼── gpudriver  (/run/nvidia/driver symlink)
                     ├── infiniband     (sysfs tree)                         └── pcibus     (NFD feature file)
...
| Surface                                                                                                    | Consumers                                                                  | Coverage                                                                                                                                                                         |
|------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `/sys/class/infiniband/mlx5_<N>/*`                                                                         | Network Operator, `ibstat`, `ibv_devinfo`, DCGM fabric metrics, Topograph  | ✓ `infiniband` via `pkg/network/mockib/render` — full HCA surface (18 file attributes + `gids/`, `pkeys/`, `counters/`, `gid_attrs/` subdirs); `gids/0` derived as `fe80::+port_guid` |
| `libibverbs` C ABI (`ibv_get_device_list`, `ibv_open_device`, `ibv_query_port`, ...)                       | RDMA-aware apps, MPI, DCGM                                                 | ✓ `infiniband` stages `libibmockverbs.so` for LD_PRELOAD                                                                                                                              |
...
| Cross-node fabric relay (TCP, `MOCK_IB=full`)                                                              | multi-node `iblinkinfo`, subnet discovery                                  | ✓ `infiniband` fabric mode via `pkg/network/mockib/fabric`                                                                                                                            |
| IB CLI tools (`ibstat`, `iblinkinfo`, `ibnetdiscover`, `ibping`, `ibv_devinfo`) — real ELFs, RPATH-patched | operator introspection, GPU Operator IB validator                          | ✓ `infiniband` stages tools RPATH-patched at Docker build time by `bundle-ib-tools.sh`                                                                                                 |
