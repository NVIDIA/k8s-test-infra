...
| IB CLI tools (`ibstat`, `iblinkinfo`, `ibnetdiscover`, `ibping`, `ibv_devinfo`) — real ELFs, RPATH-patched | operator introspection, GPU Operator IB validator                          | ✓ `infiniband` stages tools RPATH-patched at Docker build time by `stage-ib-tools.sh`                                                                                                 |
