...
| `pcibus`                | Component 2 — GPU on PCI bus                                    | PCI sysfs tree + libmockfs.so staging                                                  | NFD feature file                        | —                                                                    |
...
| `libcuda.so.<version>` C ABI (driver API)                                                                     | CUDA driver-API-only apps                                                    | ~ `gpudriver` stages the shim; only 15 functions in `shims/libcuda`. Full CUDA runtime not covered.                                                            |
...
| Kernel module presence: `/proc/modules`, `/sys/module/nvidia/{refcnt,coresize,initstate,version,holders/}`, `/sys/module/<name>/` for `nvidia_uvm`, `nvidia_modeset`, `gdrdrv`, `nvidia_fs`, plus `nvidia_peermem` and `mlx5_core` when InfiniBand is enabled | `lsmod`, GPU Operator validator's `isNvidiaModuleLoaded()` and its per-feature `lsmod` greps for `gdrdrv`, `nvidia_fs`, `nvidia_peermem` and `mlx5_core`, `modprobe` | ✓ `gpudriver` via `internal/kmod`. The agent mirrors the node's `/sys/module` and adds the NVIDIA entries to the copy. The mirror keeps built-in modules and their `parameters/`, because the mount replaces that tree for every reader, so a host module keeps its `lsmod` columns. Two channels deliver the surface: `libmockfs.so` redirects both paths for libc callers, and both CDI specs bind-mount `sys/module` at its kernel path for Go callers, which no `LD_PRELOAD` reaches. runc allows only an allowlist inside `/proc`, so nothing can bind-mount `/proc/modules`, and CDI-served containers get a generated `lsmod` that reads the mounted tree. A `State` reconcile refreshes the mirror. A module load or unload does not, so the mirror can lag the node |
...
**Delivery**: files materialized under `/host/var/lib/nvml-mock/driver/`, mounted into workload containers via CDI (`nvidia.yaml` and `nvml-mock-nri.yaml`) or NRI overlay. The driver footprint needs no LD_PRELOAD, since Go consumers bypass it. The module surface is the exception: the agent renders it outside `driver/`, at `sys/module` and `proc/modules`, and needs both channels. Only the shim can redirect `/proc/modules`, and only a mount can serve `/sys/module`.
...
| `libpci`-based tools via `libmockfs.so` LD_PRELOAD                                         | `lspci`, C-based sysfs walkers                                                                                 | ✓ `pcibus` stages the shim + owns the LD_PRELOAD entry                     |
...
**Delivery**: files under `/host/var/lib/nvml-mock/sys/` mounted at `/sys` in containers via CDI. LD_PRELOAD of `libmockfs.so` covers C `libpci` consumers.
