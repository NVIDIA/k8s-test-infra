...
| `ib`                 | Component 5 — InfiniBand HCA                                    | IB sysfs tree + libibmock*.so staging + IB CLI tool staging                                | one `dummy` netdev per HCA in the node's network namespace | `internal/ib/daemon.Server`; optional fabric relay            |
...
| Mellanox PCI devices (vendor `15b3`, class `0207`) with `driver → mlx5_core`, plus `infiniband/mlx5_<N>/` and `net/<iface>/` **directories** | RDMA shared device plugin (enumerates the bus, matches the vendor, then reads those two entries) | ✓ `pcibus` — directories rather than symlinks on purpose: the plugin lists the entry's names, so a link into the IB tree would resolve outside the mock hierarchy for no gain |
| `feature.node.kubernetes.io/pci-15b3.present=true` in the same feature file | Network Operator (its node selector for the plugin DaemonSet) | ✓ `pcibus.Apply` when the profile has HCAs — its bundled NFD cannot derive the label from the pci source, which is configured for GPU device classes only |
...
| Class entries as **symlinks** into `infiniband_devices/`, not directories                                  | any consumer that `readlink`s a class entry to reach the device            | ✓ `ib` — a directory reads identically through the entry itself, so this is a silent failure mode: the consumer resolves nothing and reports no device                        |
| `/dev/infiniband/{uverbs,umad,issm}<N>` as real character devices                                          | `libibverbs` device open, the RDMA shared device plugin's device specs     | ✓ `ib` — `mknod` under `CAP_MKNOD`; a regular file passes an existence check and then fails the `open`, the same silent shape                                                |
| A netdev per HCA in the **node's** network namespace, named from `NetdevPrefix`                             | RDMA plugins deciding an HCA is usable, `net/` resolution                  | ✓ `ib` — `dummy` links created under `CAP_NET_ADMIN` after `setns` into the node's namespace (`CAP_SYS_ADMIN` + `CAP_SYS_PTRACE`), since a `hostNetwork` consumer reads that namespace |
| The node's own `/sys/class/*` entries the mock tree hides                                                  | everything else in a served container                                      | ✓ served back as individual mounts by the NRI plugin — mounting over `/sys/class` shadows classes the mock does not own, extending #689's `/sys/devices` concern to classes   |
...
**Delivery**: the tree is served at the kernel paths — `/sys/class`, `/sys/devices`, `/sys/bus/pci/devices` — because a Go consumer reads those directly and LD_PRELOAD cannot reach it. The NRI plugin emits the mounts for every pod on the node, not only pods that request a GPU: an RDMA consumer requests none, so CDI never resolves a spec for it. Mounts are private and non-recursive; a shared bind makes the copy a peer, and container mounts then propagate onto the node and double per container generation (see the mount-table regression that wedged containerd). C-shim libs still go via LD_PRELOAD, UMAD over the Unix socket, cross-node over the optional TCP fabric relay.
