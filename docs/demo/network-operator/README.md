...
| `rdma-shared-device-plugin` (after push) | Runs, advertises **no** `rdma/*` | Static Go binary makes raw syscalls and bypasses the `LD_PRELOAD` sysfs redirect, so it reads the empty host sysfs |
...
libc-based userspace tools inside pods**, but the Network Operator's node-level
detection (NFD) and its static-Go device plugins/drivers operate below or
outside that layer, so they do not see the mocks.
