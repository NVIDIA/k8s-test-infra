...
* **NRI-delivered mock stack** — the chart stages mock files on every
  ...
  environment, and annotated IMEX channel nodes into a separate ordinary
  workload DaemonSet. The workload has no mock mounts or mock environment.
* **Real `nvidia-imex` in NO GPU mode** — the workload image fronts
...
2. Builds and loads the standard `nvml-mock:compute-domain` image, then
   builds the `nvml-mock:compute-domain-imex` workload image via the
   `workload` target of
   [`Dockerfile.compute-domain-daemon`](../../../deployments/nvml-mock/Dockerfile.compute-domain-daemon),
   which contains the real `nvidia-imex` (NO GPU mode via
   `imex-nogpu-shim`) but no mock GPU files.
...
4. Recycles the staging and NRI DaemonSets in order, then creates a separate
   `compute-domain-workload` namespace and deploys a freshly restarted ordinary
   real-IMEX DaemonSet. Its only mock-related configuration is the
   `nvml-mock.nvidia.com/imex-channels: "true"` annotation. NRI supplies the
   mock NVML overlay and per-node topology identity at container creation. The
   workload manifest also installs its own ingress NetworkPolicy, allowing TCP
   50000 and 50005 only from peer `app=compute-domain-imex` pods in that same
...
5. **Scenario 1 — per-node fabric identity**. Runs NRI-injected
   `check-fabric` inside one workload pod per worker and asserts:
...
staging, NRI, and workload DaemonSets so same-tag image rebuilds, restored
...
# 2. Build the standard mock image, then the separate real-IMEX workload
...
kind load docker-image nvml-mock:compute-domain nvml-mock:compute-domain-imex \
...
#    files and topology before any workload container is created.
...
# 7. Idempotently create the workload namespace, apply the ordinary workload
...
kubectl --context kind-nvml-mock-compute-domain -n compute-domain-workload \
    apply -f docs/demo/compute-domain/imex-agent.yaml
kubectl --context kind-nvml-mock-compute-domain -n compute-domain-workload \
    rollout restart daemonset/compute-domain-imex
kubectl --context kind-nvml-mock-compute-domain -n compute-domain-workload \
    rollout status daemonset/compute-domain-imex --timeout=180s
...
for node in nvml-mock-compute-domain-{worker,worker2,worker3,worker4}; do
  pod=$(kubectl --context kind-nvml-mock-compute-domain \
    -n compute-domain-workload get pods -l app=compute-domain-imex \
...
* **Peers never connect at all.** The demo workload has its own NetworkPolicy
  in [`imex-agent.yaml`](./imex-agent.yaml). It selects
  `app=compute-domain-imex` and admits TCP 50000 (IMEX gRPC peer) and 50005
  (command/status) only from matching peer pods in the same
  ...
  **enforces** NetworkPolicy on current kind releases, so add any new workload
  ...
  selects nvml-mock daemon pods, not this NRI-injected workload. Its IMEX port
  allowances remain for backwards compatibility and custom chart images that
  run IMEX inside the chart-managed DaemonSet.
* **Rerunning after a failed Scenario 2.** A run that dies mid-scenario
  ...
  Normal reruns restart the `compute-domain-imex` DaemonSet after refreshing
  staging and NRI, so they discard those stale processes automatically. To
  recycle the workload pods manually, run
  `kubectl --context kind-nvml-mock-compute-domain -n compute-domain-workload
  delete pods -l app=compute-domain-imex`; the DaemonSet recreates them clean.
