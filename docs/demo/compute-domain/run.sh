...
# pod_on_node: echo the NRI-injected IMEX workload pod on the requested node.
pod_on_node() {
  local node=$1
  ...
  for _ in $(seq 1 30); do
    local name
    name=$(kubectl_ctx -n "${WORKLOAD_NAMESPACE}" get pods -l "app=compute-domain-imex" \
...
info "Building NRI workload image with real nvidia-imex: ${DEMO_IMAGE_NAME}"
...
kind load docker-image "${IMAGE_NAME}" "${DEMO_IMAGE_NAME}" --name "${CLUSTER_NAME}"

###############################################################################
# Step 3 — Stage nvml-mock and deploy the NRI-injected real-IMEX workload
...
# topology ConfigMap does not rerun setup.sh or restart an existing workload.
...
info "Deploying real-IMEX workload with peer-only ingress (mock delivery is entirely NRI)"
...
# does not govern this separate workload.
kubectl_ctx -n "${WORKLOAD_NAMESPACE}" apply -f "${REPO_ROOT}/docs/demo/compute-domain/imex-agent.yaml" >/dev/null
kubectl_ctx -n "${WORKLOAD_NAMESPACE}" rollout restart daemonset/compute-domain-imex >/dev/null
kubectl_ctx -n "${WORKLOAD_NAMESPACE}" rollout status daemonset/compute-domain-imex --timeout=180s >/dev/null
...
###############################################################################
# The workload image carries the real nvidia-imex behind imex-nogpu-shim;
...
# command/status data (port 50005) across pods. The workload's NetworkPolicy
...
ok "NRI injected mock IMEX channel nodes into both real-IMEX workloads"
...
# then the workload, so every new workload container receives the new file.
sub "recycling staging pods and NRI workload pods to re-read topology"
kubectl_ctx -n "${MOCK_NAMESPACE}" delete pods -l "app.kubernetes.io/name=${RELEASE_NAME}" \
  --ignore-not-found >/dev/null
kubectl_ctx -n "${MOCK_NAMESPACE}" rollout status "daemonset/${RELEASE_NAME}" --timeout=180s >/dev/null
kubectl_ctx -n "${WORKLOAD_NAMESPACE}" delete pods -l app=compute-domain-imex \
  --ignore-not-found >/dev/null
kubectl_ctx -n "${WORKLOAD_NAMESPACE}" rollout status daemonset/compute-domain-imex --timeout=180s >/dev/null
