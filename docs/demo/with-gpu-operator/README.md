...
> anywhere on the cluster, naming the release and printing the `helm uninstall`
> that clears it. The lookup is cluster-wide because the state at risk is
> per-node and no namespace scopes it, and because the three demos default to
> three different namespaces. `DEMO_ASSUME_YES=true` overrides, with a warning that says what is
...
| `DEMO_ASSUME_YES` | `false` | Proceed without the confirmation prompt off a throwaway cluster, and past the co-location refusals. |

Exit codes come from the shared preflight: `2` no usable cluster, `3` missing
or too-old tooling, `4` confirmation declined or impossible, another nvml-mock
demo already installed anywhere on the cluster, or helm unable to list releases
to rule that out, `5` a configuration the chart cannot express (a digest-pinned
`NVML_MOCK_IMAGE`).
