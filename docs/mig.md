...
| `nvml_profile` | Which NVML profile the row is, as the suffix of the `NVML_GPU_INSTANCE_PROFILE_*` constant: `1_SLICE`, `1_SLICE_REV1`, `2_SLICE`, `7_SLICE`. It is what `nvmlDeviceGetGpuInstanceProfileInfo` takes as its `profile` argument, and what every lookup keys on — where `profile_id` below is the number that same call *returns*. An unknown suffix, or one declared twice, is refused at load |
...
### What the REV suffixes distinguish

Three enum suffixes name variants of the same compute width, and what separates
them is not compute.

`1_SLICE` is the plain one-slice partition. `1_SLICE_REV1` is the **media
extension**, which the listing spells `+me`: the same slice and the same
framebuffer, but holding the board's video engines. That is why `a100`'s
`1g.5gb` declares no `decoders` while `1g.5gb+me` declares a decoder, a JPEG
engine and an OFA on identical compute and memory, and why it offers
`instances: 1` — a board has one such set to give away. `1_SLICE_REV2` is the
**double-memory** variant: still one compute slice, but two memory units rather
than one, so it is named for the larger framebuffer it holds — `1g.10gb` on
`a100`, `1g.45gb` on `b200` — and only four fit a seven-slice board.

The compute instance enums reuse the `REV1` suffix for the same idea applied to
a different object, so a `compute_instances` entry naming it is the
media-capable compute instance shape rather than a `+me` GPU instance. It
reports video engines only when the GPU instance it sits in has them to share:
the entry under `a100`'s `1g.5gb` declares none, and the identically named entry
under `1g.5gb+me` declares all three.
...
| Field | Meaning |
|---|---|
| `nvml_profile` | The same key as above bound to the other enum family: the suffix of the `NVML_COMPUTE_INSTANCE_PROFILE_*` constant that `nvmlGpuInstanceGetComputeInstanceProfileInfo` takes. The two families share their suffix vocabulary, so which of them a value names is decided by the block it sits in rather than by the value. Unknown, or declared twice within one row, is refused |
...
Transcribe rows from
[Supported MIG Profiles](https://docs.nvidia.com/datacenter/tesla/mig-user-guide/latest/supported-mig-profiles.html)
in NVIDIA's MIG user guide, which is where every shipped profile's rows come
from — each names its table in a YAML comment. It has no GB200 or GB300 table,
so those two are taken from what the boards themselves report. NVIDIA publishes
placements only as diagrams, so those are transcribed from the layout the
diagrams show.
