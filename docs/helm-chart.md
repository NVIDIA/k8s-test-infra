...
| `imex.nodeSoftware.enabled` | `true` | Download the architecture-specific IMEX archive pinned by version and SHA-256, cache it on the node, and stage its daemon, CLI, config, and Mokka shim in the driver tree. Requires outbound HTTPS from the node-agent pod on the first successful stage; set `false` to opt out |
