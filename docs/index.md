...
| **IMEX** | Real IMEX userspace peers and control commands in NO GPU mode, plus simulated channel devices and driver capability files. The same multi-platform Mokka image serves ordinary and compute-domain installations; IMEX remains dormant until started. | ComputeDomain controller integration, peer readiness, domain membership and liveness handling | No GPU memory exchange, data path, bandwidth, latency or hardware validation. |
