...
`MOCK_FABRICMANAGER_STATE_DIR`, `IMEX_USERSPACE_ENABLED` and
...
`IMEX_USERSPACE_ENABLED=true` makes the IMEX simulator download the
...
tool, config, and Mokka shim in the driver tree. The default is disabled and
