...
It also ships `/usr/local/bin/setup-soft-roce.sh` plus a `soft-roce.service`
systemd unit that runs it automatically at **node boot**: it loads `rdma_rxe`,
puts the RDMA subsystem in exclusive-netns mode, and creates an `rxe0` link over
a netdev (default `eth0`). The demo does not `docker exec` into nodes. You can
override the netdev/link name via the `RXE_NETDEV` / `RXE_LINK` env vars (e.g. a
systemd drop-in) or by invoking the script directly (`setup-soft-roce.sh
<netdev> <link>`).
