...
test("GitHub client rejects a paginated collection above its fixed bound", async () => {
  const { createGitHubClient } = require("../src/github-client.js");
  const octokit = {
    rest: { issues: { listLabelsForRepo: async () => ({ data: [] }) } },
    async paginate() {
      return Array.from({ length: 1001 }, (_, index) => label(`generated-${index}`));
    },
  };
  const github = createGitHubClient(octokit, "nvidia", "k8s-test-infra");

  await assert.rejects(() => github.listLabels(), /listLabels.*1000/i);
});
...
test("GitHub client redacts credentials from the actual Octokit error shape", async () => {
  const { createGitHubClient } = require("../src/github-client.js");
  const authorization = "token octokit-sensitive-sentinel-9347"; // gitleaks:allow -- synthetic redaction fixture
  const alternateToken = "alternate-sensitive-sentinel-6218"; // gitleaks:allow -- synthetic redaction fixture
...
test("summary serialization rejects output above 64 KiB", () => {
  const { serializeSummary } = require("../src/index.js");

  assert.equal(serializeSummary({ status: "ok" }), '{"status":"ok"}');
  assert.throws(
    () => serializeSummary({ value: "a".repeat((64 * 1024) + 1) }),
    /summary.*65536/i,
  );
});
