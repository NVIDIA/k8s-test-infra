...
test("bounds aliases, owner entries, changed paths, and owner declarations", async (t) => {
  const manyAliases = [
    "aliases:",
    ...Array.from({ length: 501 }, (_, index) => `  group_${index}: [alice]`),
    "",
  ].join("\n");
  const manyMembers = `aliases:\n  group: [${Array.from({ length: 101 }, (_, index) => `member-${index}`).join(", ")}]\n`;
  const manyOwners = `reviewers: [${Array.from({ length: 1001 }, (_, index) => `reviewer-${index}`).join(", ")}]\n`;
  const ownerFile = parseOwnersFile("reviewers: [alice]\napprovers: [bob]\n", "/OWNERS");
  const cases = [
    ["alias groups", () => parseAliases(manyAliases), /aliases.*500/i],
    ["alias members", () => parseAliases(manyMembers), /alias group.*100/i],
    ["owners", () => parseOwnersFile(manyOwners, "/OWNERS"), /reviewers.*1000/i],
    [
      "changed paths",
      () => resolveOwners(
        Array.from({ length: 1001 }, (_, index) => `src/${index}.go`),
        [ownerFile],
        new Map(),
        resolutionPolicy(["/OWNERS"]),
      ),
    ...
    [
      ...
      () => resolveOwners(
        ["src/main.go"],
        Array.from({ length: 101 }, () => ownerFile),
        new Map(),
        resolutionPolicy(["/OWNERS"]),
      ),
      /ownerFiles.*100/i,
    ...
    [
      ...
      () => resolveOwners(
        ...
        [ownerFile],
        new Map(),
        resolutionPolicy(Array.from({ length: 101 }, (_, index) => `/path-${index}/OWNERS`)),
      ),
      /activeOwnerFiles.*100/i,
    ],
  ];
