...
test("bounds changed paths, area rules, patterns, and labels", async (t) => {
  const cases = [
    [
      "changed paths",
      () => deriveAreaLabels(
        Array.from({ length: 1001 }, (_, index) => `docs/${index}.md`),
        currentAreas,
      ),
      /paths.*1000/i,
    ],
    [
      ...
      () => deriveAreaLabels(
        ["docs/guide.md"],
        areaConfig(Array.from({ length: 501 }, () => currentAreas.areas[0])),
      ),
      /rules.*500/i,
    ],
    [
      ...
      () => deriveAreaLabels(
        ["docs/guide.md"],
        areaConfig([{
          paths: Array.from({ length: 101 }, (_, index) => `docs/${index}/**`),
          labels: ["area/docs"],
        }]),
      ),
      /paths.*100/i,
    ...
    [
      ...
      () => deriveAreaLabels(
        ...
        areaConfig([{
          paths: ["docs/**"],
          labels: Array.from({ length: 21 }, (_, index) => `area/${index}`),
        }]),
      ),
      /labels.*20/i,
    ],
