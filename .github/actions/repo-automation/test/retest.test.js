...
test("plans only configured source-controlled pull-request CI for this repository and PR", () => {
  const result = planRetest(input({
    runs: [
      run(1),
      run(2, { workflowPath: ".github/workflows/basic-checks.yaml" }),
      run(3, { workflowPath: ".github/workflows/helm.yaml" }),
      run(4, { workflowPath: ".github/workflows/custom-pr.yml" }),
      run(5, { workflowPath: ".github/workflows/release.yaml" }),
    ],
    workflowAllowlist: [
      ".github/workflows/automation-ci.yml",
      ".github/workflows/custom-pr.yml",
    ],
  }));
...
test("rejects non-pull-request, wrong-repository, and unsafe workflow identities", () => {
  const result = planRetest(input({
    runs: [
      run(1),
      run(2, { workflowPath: ".github/workflows/basic-checks.yaml" }),
...
test("validates all planner and persisted-state fields without throwing", () => {
  const invalidInputs = [
    ...
    input({ workflowAllowlist: [] }),
    input({ workflowAllowlist: [".github/workflows/automation-ci.yml", ".github/workflows/automation-ci.yml"] }),
    input({ workflowAllowlist: ["../automation-ci.yml"] }),
    input({ workflowAllowlist: [".github/workflows/automation-ci.yml@main"] }),
    input({ workflowAllowlist: Array(33).fill(".github/workflows/automation-ci.yml") }),
