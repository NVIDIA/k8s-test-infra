...
test("uses lexical ties independently of owner, repository, and PR seed fields", () => {
  const candidates = Array.from({ length: 24 }, (_, index) => `reviewer-${String(index + 1).padStart(2, "0")}`);
...
test("breaks equal uncovered-file coverage by normalized lexical login", () => {
  const candidates = ["carol", "bob", "alice"];
  const files = [
  ...
  assert.deepEqual(selectReviewers({
    candidates,
    files,
    seed: { owner: "nvidia", repo: "k8s-test-infra", pr: 73 },
    target: 2,
    author: "author",
    requested: [],
  }).selected, ["alice", "bob"]);
...
test("bounds candidates, requested reviewers, changed files, and per-file reviewers", async (t) => {
  const logins = Array.from({ length: 1001 }, (_, index) => `reviewer-${index}`);
  const files = Array.from(
    { length: 1001 },
    (_, index) => changedFile(`src/${index}.go`, ["alice"]),
  );
  const cases = [
    ["candidates", () => selection({ candidates: logins }), /candidates.*1000/i],
    ["requested", () => selection({ requested: logins }), /requested.*1000/i],
    ["files", () => selection({ files }), /files.*1000/i],
    [
      "file reviewers",
      () => selection({ files: [changedFile("src/main.go", logins)] }),
      /file reviewers.*1000/i,
    ],
  ];
