...
function requireNonEmptyString(value, configPath, errors, maximumLength) {
  if (typeof value !== "string" || value.trim() === "") {
    addError(errors, configPath, "must be a non-empty string");
    return false;
  }
  if (maximumLength !== undefined && value.length > maximumLength) {
    addError(errors, configPath, `must not exceed ${maximumLength} characters`);
    return false;
  }
  return true;
}

function requireStringArray(value, configPath, errors, maximumItems) {
  if (!Array.isArray(value) || value.length === 0) {
    addError(errors, configPath, "must be a non-empty array");
    return false;
  }
  if (maximumItems !== undefined && value.length > maximumItems) {
    addError(errors, configPath, `must not exceed ${maximumItems} items`);
  }
...
function validatePolicy(policy, errors) {
  ...
  if (requireRecord(policy.review, "policy.review", errors)) {
    ...
    } else if (policy.review.reviewerTarget > MAX_REVIEWERS) {
      addError(
        errors,
        "policy.review.reviewerTarget",
        `must not exceed ${MAX_REVIEWERS}`,
      );
    }
  }
  ...
  if (!Array.isArray(policy.bots) || policy.bots.length === 0) {
    ...
    if (policy.bots.length > MAX_BOTS) {
      addError(errors, "policy.bots", `must not exceed ${MAX_BOTS} items`);
    }
    for (let index = 0; index < policy.bots.length; index += 1) {
      ...
      requireStringArray(bot.emails, `${botPath}.emails`, errors, MAX_BOT_EMAILS);
    }
  }
...
function validateLabels(labels, errors) {
  ...
  if (labels.labels.length > MAX_LABELS) {
    addError(errors, "labels.labels", `must not exceed ${MAX_LABELS} items`);
  }
...
function validateAreas(areas, declaredLabels, errors) {
  ...
  if (areas.areas.length > MAX_AREA_RULES) {
    addError(errors, "areas.areas", `must not exceed ${MAX_AREA_RULES} items`);
  }
  ...
  for (let index = 0; index < areas.areas.length; index += 1) {
    ...
    requireStringArray(area.paths, `${areaPath}.paths`, errors, MAX_PATTERNS_PER_AREA);
