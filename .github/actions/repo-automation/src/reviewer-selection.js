...
const { MAX_CHANGED_FILES, MAX_REVIEWERS } = require("./limits.js");
...
function normalizeLogins(values, field) {
  ...
  if (values.length > MAX_REVIEWERS) {
    throw new TypeError(`${field} must not exceed ${MAX_REVIEWERS} items`);
  }
...
function normalizeFiles(files) {
  ...
  if (files.length > MAX_CHANGED_FILES) {
    throw new TypeError(`files must not exceed ${MAX_CHANGED_FILES} items`);
  }
...
function candidateDetails(candidates, files) {
  const details = [];
...
function chooseCandidate(remaining, uncovered) {
  ...
  const covering = maximumCoverage > 0
    ...
    : ranked;
  return covering.sort((left, right) => compareText(left.login, right.login))[0];
}
