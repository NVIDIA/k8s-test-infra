...
function parseOwnerList(value, field) {
  ...
  if (value.length > MAX_OWNER_ENTRIES) {
    throw new TypeError(`${field} must not exceed ${MAX_OWNER_ENTRIES} items`);
  }
...
function validateAliasMap(aliases) {
  ...
  for (const [name, members] of aliases) {
    ...
    if (members.length > MAX_ALIAS_MEMBERS) {
      throw new TypeError(`alias group must not exceed ${MAX_ALIAS_MEMBERS} members`);
    }
...
function parseAliases(text) {
  ...
  if (Object.keys(value.aliases).length > MAX_ALIAS_GROUPS) {
    throw new TypeError(`aliases must not exceed ${MAX_ALIAS_GROUPS} groups`);
  }
...
function normalizePolicy(policy) {
  ...
  if (policy.activeOwnerFiles.length > MAX_ACTIVE_OWNER_FILES) {
    throw new TypeError(
      `policy.activeOwnerFiles must not exceed ${MAX_ACTIVE_OWNER_FILES} items`,
    );
  }
...
function activeDeclarations(ownerFiles, activeOwnerFiles) {
  ...
  if (ownerFiles.length > MAX_OWNER_FILES) {
    throw new TypeError(`ownerFiles must not exceed ${MAX_OWNER_FILES} items`);
  }
...
function resolveOwners(paths, ownerFiles, aliases, policy) {
  ...
  if (paths.length > MAX_CHANGED_FILES) {
    throw new TypeError(`paths must not exceed ${MAX_CHANGED_FILES} items`);
  }
