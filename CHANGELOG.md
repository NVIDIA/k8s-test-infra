...
- e2e: a chart profile whose `device_defaults.architecture` is missing or names
  no recognized generation now fails to load. It previously loaded with an
  architecture nothing recognized, leaving each "this generation and newer"
  expectation to answer from a default rather than from the profile.
